...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($orgUserId && in_array($action, ['approve', 'reject'])) {
        if ($action === 'approve') {
                            ...
                            <td>
                                <?php if (!$org['is_verified']): ?>
                                    <form method="POST" style="display:inline-flex;gap:.35rem">
                                        ...
                                        <button name="org_action" value="approve" class="btn btn-success btn-sm">Approve</button>
                                        <button name="org_action" value="reject" class="btn btn-danger btn-sm" data-confirm="Reject and remove this organizer?">Reject</button>
                                    </form>
                                <?php else: ?>
